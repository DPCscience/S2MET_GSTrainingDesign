...
# Plot per trait
traits <- unlist(distinct(mds_method, trait))
...
for (i in seq_along(traits)) {
  ...
  tr <- traits[i]
   ...
   g <- mds_method %>%
    ...
    geom_point() +
    facet_wrap(~ method, scales = "free", ncol = 5) +
    theme(
...
gg_list1 <- map(gg_list, ggplotGrob)
...
(g_cluster_varcomp <- full_join(cluster_varcomp, gc_lrt) %>% 
  filter(grp %in% c("line_name", "line_name:cluster")) %>%
  mutate(k = as.factor(k),
         grp = parse_factor(grp, levels = c("line_name", "line_name:cluster")),
