...
# 
# # Plot per trait
# traits <- as.character(unique(mds_method$trait))
# 
# # List to store ggpots
# gg_list <- list()
# 
...
#     theme(
#       axis.text = element_text(size = 5),
#       plot.margin = unit(x = c(0, 0.1, 0, 0.1), units = "cm")
#     )
#   
#   if (i == 1) {
#     g <- g + 
...
# }
# 
# (g_merge <- cowplot::plot_grid(plotlist = gg_list, ncol = 1, align = "v", 
#                               axis = "tblr"))
...
add_one_env_pred_summ <- add_one_env_pred_stand %>% 
  ...
  summarize(mean_cor = mean(cor), sd_cor = sd(cor), avg_dist = mean(distance)) %>%
...
add_one_env_pred_summ %>%
  # filter(environment == "CRM16") %>%
  ...
  xlab("Cumulative Environmental Distance")
