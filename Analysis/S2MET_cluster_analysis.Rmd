...
# Plot
(g_herit_clust <- cluster_herit_all_use %>% 
    mutate(k = parse_number(k)) %>%
    ggplot(aes(x = k, y = estimate, col = method, shape = type)) + 
    geom_point() + 
    geom_line() + 
    facet_grid(. ~ trait) +
    labs(
...
cluster_lrt <- cluster_herit_all_use %>% 
  mutate(k = parse_number(k)) %>%
  select(trait:mod, llik) %>%
  mutate(llik = map_dbl(llik, as.numeric)) %>%
  spread(mod, llik) %>%
  mutate(lr = (-2 * (no_gc - full)),
         p_value = pchisq(q = lr, df = 1, lower.tail = FALSE) / 2,
         adj_p_value = p.adjust(p_value, method = "fdr"),
         neg_log_adj_p = -log10(adj_p_value),
         neg_log_fdr001 = -log10(0.001))

# Plot the neg-log p value by cluster, trait, and method
(g_cluster_lrt <- cluster_lrt %>%
  ggplot(aes(x = k, y = neg_log_adj_p, col = method)) +
  geom_point() +
  # geom_hline(aes(yintercept = neg_log_fdr001)) +
  facet_grid(trait ~ .) +
  ylab("-log10(p)") +
  xlab("K") +
  labs(
    title = "Significance of Genotype-Cluster Interaction Variance",
    subtitle = "Significance measured by a likelihood ratio test"
  ))

# Save it
save_file <- file.path(fig_dir, "geno_cluster_lrt.jpg")
ggsave(filename = save_file, plot = g_cluster_lrt, width = 8, height = 6)
...
(g_cluster_varcomp <- cluster_varcomp %>% 
  filter(grp %in% c("line_name", "line_name:cluster")) %>%
  mutate(grp = parse_factor(grp, levels = c("line_name", "line_name:cluster"))) %>%
...
load(file.path(result_dir, "S2MET_pred_by_random_env.RData"))
...
## Manipulate the random environment experiment such that replications of a prediction 
## environment are averaged
random_add_env <- random_env_pred_out %>%
  bind_rows() %>% 
  unnest(pred_out) %>% 
  group_by(trait, pred_env, n_train) %>% 
  summarize(pred_acc = mean(cor, na.rm = T), sd_acc = sd(cor, na.rm = T))


# Summarize across all environments for a method
add_acc_summ <- add_one_env_pred_stand %>% 
  group_by(trait, method, env_add) %>% 
  summarize(pred_acc = mean(cor), sd_pred_acc = sd(cor, na.rm = T),
...
Plot using the summaries


```{r plot.sum}
## Plot the summary from adding more distant environments

(g_add_one_env <- add_acc_summ %>%
  # filter(pred_env == "STP16") %>%
  ggplot(aes(x = cum_mean_scale)) +
  geom_point(aes(y = pred_acc, col = method, shape = method)) +
  geom_line(aes(y = pred_acc, col = method, shape = method)) +
  # geom_ribbon(aes(ymin = mean_cor - sd_cor, ymax = mean_cor + sd_cor), alpha = 0.01, lwd = 0.25) +
...
ggsave(filename = save_file, plot = g_add_one_env, width = 8, height = 6, dpi = 500)


## Summarize the random predictions
add_random_summ <- random_add_env %>% 
  group_by(trait, n_train) %>% 
  summarize(sd_pred_acc = sd(pred_acc, na.rm = T), 
            pred_acc = mean(pred_acc)) %>% 
  rename(env_add = n_train) %>% 
  mutate(method = "Random", env_add = as.character(env_add)) %>%
  bind_rows(., add_acc_summ) %>%
  ungroup() %>%
  mutate(env_add = parse_factor(env_add, levels = sort(unique(as.numeric(env_add)))))


## Plot the random line
(g_add_rand_env <- add_random_summ %>%
  ggplot(aes(x = env_add, y = pred_acc, col = method, shape = method, group = method)) +
  geom_point() +
  geom_line() +
  # geom_ribbon(aes(ymin = mean_cor - sd_cor, ymax = mean_cor + sd_cor), alpha = 0.01, lwd = 0.25) +
  # geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = FALSE) +
  facet_grid(trait ~ ., scales = "free_y") +
  ylab("Prediction Accuracy") +
  xlab("Number of Environments Used in Prediction") +
  labs(
    title = "Prediction Accuracy Upon Adding More Distant Environments"
  ) +
  theme_bw())
