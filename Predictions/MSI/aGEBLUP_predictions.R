...
a_geblup2_results <- data_frame(trait = traits, results = replicate(length(traits), data_frame(environment = env_pred), simplify = FALSE))


# Iterate over all traits
for (i in seq(nrow(a_geblup2_results))) {
  
  a_geblup2_results$results[[i]] <- a_geblup2_results$results[[i]] %>%
    group_by(environment) %>%
    do(pred_out = {
      
      # Training and prediction phenotypes
      pheno_train <- filter(pheno, line_name %in% tp, environment != .$environment, trait == a_geblup2_results$trait[i])
      pheno_pred <- filter(pheno, line_name %in% vp, environment == .$environment, trait == a_geblup2_results$trait[i])
...
save_file <- file.path(MSI_proj_dir, "aGE-BLUP_results2.RData")
list_to_save <- c("a_geblup2_results")
