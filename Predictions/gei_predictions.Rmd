...
sherlock_time <- system.time({
  solve_out <- 
    SherLMMock::SherLMMock(y = y_train, 
                           X = X, 
                           Zlist = list(Z_g, Z_e, Z_ge),
                           Glist = list(G, E_1, O_1))
...
a_geblup2_results <- data_frame(trait = traits, 
...
a_geblup2_analysis %>%
  ...
  xlab("Environment") +
  labs(
    title = "Additive GE-BLUP2 Prediction Accuracy",
    subtitle = "All Training Environments Except Validation Environment Used to Predict Each Validation Environment"
  )

# Combine
bind_rows(a_geblup2_analysis, gblup2_analysis) %>%
