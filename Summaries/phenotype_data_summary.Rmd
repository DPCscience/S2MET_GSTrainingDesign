...
2. Correlations among environments
...
# Group by trait and environment
var_comp_env <- s2_met_tidy_model %>%
  group_by(trait, environment) %>%
  # Calculate the average number of replications
  mutate(avg_rep = pbr:::avg_reps(table(line_name))) %>%
  group_by(avg_rep, add = TRUE) %>%
  do({
    # Fit the model
    fit <- lmer(value_rescale ~ check + (1|line), data = .)
    
    # Extract variances and return variance components
    as.data.frame(VarCorr(fit)) %>%
      select(grp, vcov) %>% 
      spread(grp, vcov) %>% 
      rename(V_G = line, V_R = Residual)
  })


# Group by trait and calculate variance components
var_comp <- s2_met_tidy_model %>%
  group_by(trait) %>%
  # Calculate the average number of replications and other counts
  mutate(e = n_distinct(environment),
         l = n_distinct(location),
         y = n_distinct(year),
         avg_rep = pbr:::avg_reps(table(line_name)) / e) %>%
  group_by(e, l, y, avg_rep, add = TRUE) %>%
  do({
    # Fit the model
    fit <- lmer(value_rescale ~ check + (1|line) + environment + (1|line:location) + (1|line:year) + (1|line:location:year), data = .)
    
    # Extract variances and return variance components
    as.data.frame(VarCorr(fit)) %>%
      select(grp, vcov) %>% 
      spread(grp, vcov) %>% 
      rename(V_G = line, V_GL = `line:location`, V_GY = `line:year`, V_GLY = `line:location:year`, V_R = Residual)
  })
...
where $e$ is the number of environments and $r$ is the number of replicates. In the case of an augmented design, where the number of replicates is uneven, the following formula is used to calculate the average number of replications:

$$
r = \frac{ R_1 - \frac{R_2}{R_1} }{n - 1}
$$

where $R_1 = \sum r_i$, $R_2 = \sum r^2_i$, $r_i$ is the number of replications of the $i$th entry, and $n$ is the number of entries (i.e. $n_g + n_c$).
...
herit_env <- model_vars_env %>%
  group_by(trait, environment) %>%
  summarize(H = V_G / (V_G + (V_R / avg_rep)))

    

# Plot
herit_env %>%
  ggplot(aes(x = environment, y = H)) +
  geom_col() +
